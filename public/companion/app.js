(function () {
  ...
  function answerMetrics() {
    ...
    return {
      ...
      readinessCoverage: clamp(percent(answeredIds.length, Math.min(600, QUESTIONS.length)), 0, 100)
    };
  }
  function readiness() {
  ...
  function selectPracticeQuestions(criteria, size) {
    var allCriteria = { set: "all", domain: criteria.domain, approach: criteria.approach };
  ...
  function exportProgress() {
    var exportData = { exportedAt: new Date().toISOString(), app: "S3 PMP Study Companion v3", librarySize: QUESTIONS.length, settings: state.settings, progress: state, summary: { readiness: readiness(), answers: answerMetrics(), schedule: scheduleMetrics(), totalMinutes: totalMinutes() } };
  ...
  function init() {
    if (QUESTIONS.length !== 3000) console.warn("Expected 3,000 questions; found", QUESTIONS.length);
    bindEvents();
    refreshAll();
