(function () {
  ...
  var STORAGE_KEY = "s3-pmp-companion-v2";
  ...
  function readiness() {
    ...
    return Math.round((answers.recentAccuracy * .60) + (answers.coverage * .25) + (plan.percent * .15));
  }
  ...
  function deterministicMix(items) {
    return items.slice().sort(function (a, b) {
      var aScore = ((a.id * 37) + state.history.length * 13) % 101;
      var bScore = ((b.id * 37) + state.history.length * 13) % 101;
      return aScore - bScore;
    });
  }
  function startQuickQuiz(criteria, size) {
    showView("practice");
    var pool = getQuestionPool(criteria);
    if (!pool.length && criteria.set !== "all") {
      criteria.set = "all";
      pool = getQuestionPool(criteria);
    }
    beginQuiz(deterministicMix(pool).slice(0, size));
  }
  ...
  function submitAnswer(event) {
    ...
    if (!selected) { toast("Choose an answer before submitting."); return; }
    var question = quiz.questions[quiz.index];
    var selectedIndex = Number(selected.value);
    var correct = selectedIndex === question.answer;
    quiz.answeredCurrent = true;
    quiz.results.push({ id: question.id, correct: correct, domain: question.domain });
    var previous = state.answers[question.id];
    state.answers[question.id] = { correct: correct, selected: selectedIndex, attempts: (previous ? previous.attempts : 0) + 1, lastAt: new Date().toISOString() };
    state.history.push({ id: question.id, correct: correct, selected: selectedIndex, date: new Date().toISOString() });
    ...
    choices.forEach(function (choice, index) {
      ...
      if (index === selectedIndex && !correct) choice.classList.add("incorrect");
    });
    var banner = byId("resultBanner");
    banner.className = "result-banner " + (correct ? "correct" : "incorrect");
    banner.textContent = correct ? "Correct — strong judgment" : "Not quite — best answer: " + String.fromCharCode(65 + question.answer);
  ...
  function startScheduledSession(sessionId) {
    ...
    var size = session.activity.indexOf("180-question") !== -1 ? 100 : session.activity.indexOf("25-question") !== -1 ? 25 : session.activity.indexOf("50-question") !== -1 ? 50 : 10;
    if (session.activity.indexOf("180-question") !== -1) toast("This app opens its full 100-question bank; use an official 180-question mock for exact exam pacing.");
    startQuickQuiz({ set: session.filter.set, domain: session.filter.domain, approach: session.filter.approach }, size);
  }
  function exportProgress() {
    var exportData = { exportedAt: new Date().toISOString(), app: "S3 PMP Study Companion v2", settings: state.settings, progress: state, summary: { readiness: readiness(), answers: answerMetrics(), schedule: scheduleMetrics(), totalMinutes: totalMinutes() } };
  ...
  function bindEvents() {
    ...
    byId("buildQuizButton").addEventListener("click", function () {
      var criteria = { set: byId("questionFilter").value, domain: byId("domainFilter").value, approach: byId("approachFilter").value };
  ...
  function init() {
    if (QUESTIONS.length !== 100) console.warn("Expected 100 questions; found", QUESTIONS.length);
    bindEvents();
    refreshAll();
